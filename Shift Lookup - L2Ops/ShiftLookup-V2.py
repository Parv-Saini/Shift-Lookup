...
class main_class():
    ...
    def get_roster_file(self):
        ...
        self.root = "Roster.xlsx"
        #self.root = filedialog.askopenfilename(initialdir = os.getcwd(), title = 'Select file', filetypes = [("Excel files","*.xlsx")])
    ...
    def get_shift_person(self):
        ...
        try:
            ...
            msg.showerror('File not found error!', 'Please Load the roster file first')
    ...
    def click_me(self, row_variable):
        ...
        for d in self.s1, self.s2, self.s3:                         #Iterating all the shift lists to get Shift Members
            ...
            self.new_frame = ttk.LabelFrame(self.tab1, text = frame_header)          #Creating shift frames dynamically
            self.new_frame.grid(column = counter, row = row_variable, padx = 10, sticky = tk.W)
            column_number = 1
            
            for row_number in range(len(d)):                                    #Creating the Data labels dynamically
                shift_member = str(d[row_number])
                self.data_label = ttk.Label(self.new_frame, text = shift_member, style = 'my.TLabel')
    ...
    def create_widgets(self):
        ...
        self.button1 = ttk.Button(self.tab1, text = "Click to Load the Shift Roster", command = self.get_roster_file)
        ...
        self.text_box.grid(column = 0, row = 4, padx = 20, pady = 20, sticky = 'WE', columnspan = 2)
        
        #Adding a Button for loading shift data
        self.button3 = ttk.Button(self.tab1, text = "Get Data for Specified Date", command = self.get_custom_date)
        self.button3.grid(column = 2, row = 4, padx = 20, pady = 20, sticky = 'WE')
        ...
        self.warn.grid(column = 0, row = 20, columnspan = 3, padx = 20, pady = 5)
        
        self.button4 = ttk.Button(self.tab1, text = 'Screen Refresh?', command = self.clear_screen)
        self.button4.grid(column = 0, row = 9, padx = 20, pady = 20, sticky = 'WE', columnspan = 3)
        
        self.time_zone = ['IST - 06:00 to 15:00', 'CST - 18:30 to 03:30', 'IST - 14:00 to 23:00', 'CST - 02:30 to 11:30', 'IST - 22:00 to 07:00', 'CST - 10:30 to 19:30']
        aa=0
        for a in range(3):
            frame_header = 'Shift - ' + str(a+1)
            time_frame = ttk.LabelFrame(self.tab1, text = frame_header)
            time_frame.grid(column = a, row = 10, padx = 10, sticky = 'WE')
            for x in range(2):
                time_label = ttk.Label(time_frame, text = self.time_zone[aa], style = 'my.TLabel')
                time_label.grid(column = 0, row = x, sticky = 'W', padx = 15, pady = 4)
                aa=aa+1
        #self.action.focus()
